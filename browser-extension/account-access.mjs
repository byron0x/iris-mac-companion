...
export function createAccess(browser,fetchImpl=fetch,now=Date.now) {
  ...
  async function claim(ticket) {
    ...
    await browser.storage.local.remove(['companionAccess','pendingEarnCompletion','scanRequestID']);await browser.storage.local.set({companionAccount:c});
    return status(true);
  }
  ...
  async function flushCompletion() {
    const {companionAccount:c,pendingEarnCompletion:p}=await browser.storage.local.get(['companionAccount','pendingEarnCompletion']);
    if(!c||!p||p.id!==c.id)return;
    await request({kind:'browser',id:c.id,action:'complete',runId:p.runId},c.token);
    const current=await browser.storage.local.get(['companionAccount','pendingEarnCompletion']);
    if(current.companionAccount?.id===c.id&&current.pendingEarnCompletion?.runId===p.runId)await browser.storage.local.remove('pendingEarnCompletion');
  }
  async function complete() {
    const {companionAccount:c,scanRequestID:runId}=await browser.storage.local.get(['companionAccount','scanRequestID']);
    if(!c||!runId)return;
    await browser.storage.local.set({pendingEarnCompletion:{id:c.id,runId}});
    await browser.storage.local.remove('scanRequestID');
    await flushCompletion();
  }
  async function disconnect() {
    const {companionAccount:c}=await browser.storage.local.get('companionAccount');
    await browser.storage.local.remove(['companionAccount','companionAccess','scanRequestID','pendingEarnCompletion']);
    if(c)await request({kind:'browser',id:c.id,action:'disconnect'},c.token).catch(()=>{});
  }
  return {status,claim,reserve,complete,disconnect};
}
